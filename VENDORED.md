...
| `clouddrive_common/ui/addon.py` (`356de0d`), `clouddrive_common/ui/utils.py` (`c3a1445`), `resources/skins/default/1080i/pin-dialog.xml` (`345999a`) | Line endings were converted from CRLF to LF across the whole of each file, and have since been **converted back**. **No line-ending difference from upstream remains, in any vendored file** | The conversion was invisible in review and total in a byte diff: upstream holds all three as CRLF, so every line of all three differed and a plain `git diff` showed each file as wholly rewritten while saying nothing about what had changed. Repaired rather than documented around, because the root cause turned out to be larger than three files — this branch carried no `.gitattributes`, so end-of-line conversion was decided by each developer's `core.autocrlf`, and the build reads the working tree. `.gitattributes` now carries `* -text` and the archive is a function of the commit again; the reasoning is in that file. Note for anyone re-taking the copy: upstream is **not** uniform — `db.py`, `export.py` and `remote/provider.py` arrived LF and the rest of the package CRLF — so the target is per file, and "normalise everything to LF" would rewrite most of the package and destroy exactly the property this table exists to preserve |

The adapter those imports name moved in Phase 2, from `resources/lib/auth_context.py` to
`resources/lib/kodi/auth_context.py`, which is the only directory `CI-01` permits to import `xbmc*` at
module level. Two vendored files carry the new dotted path — `remote/provider.py` and `ui/addon.py` —
and **both already have a row above**, so this is one line changed inside an existing modification
record rather than a fresh divergence from upstream: no vendored file diverges here that did not
diverge before. The boundary that made the move necessary is now held by
`tests/test_layering_gates.py`, which parses each tracked module and permits a module-level Kodi
import only under `resources/lib/kodi/` or under one of two named exemptions — `ui/addon.py` and
`ui/dialog.py`, both of which are Kodi from their first line by design and are kept that way because
the vendored tree is unmodified except where a row above says otherwise.
...
| `clouddrive_common/ui/addon.py` | In `_list_folder`, `self._load_target = item['folder']['child_count']` became a doubly-guarded read through `Utils.get_safe_value` with a numeric default | The two subscripts were chained behind nothing but `if item:`, and `_child_count_supported` is `True`, so **every** folder listing calls `get_item` and then indexes both levels unconditionally. An item that came back without a `folder` facet — a reshaped body, a truncated response, or a path that resolved to a file — raised `KeyError` here, inside the vendored tree, which surfaces as a failed listing carrying a traceback from a file this project did not write. This is the fourth of the unguarded reads BROWSE-07 names and the only one outside `resources/lib/provider/onedrive.py`; the other three moved into `resources/lib/graph/items.py`, where every field including `id` now reads through the same truth test. `Utils.get_safe_value` rather than a new helper, for the reason the rest of this repository uses it: a second truth-test implementation is the drift the gate design exists to prevent |
