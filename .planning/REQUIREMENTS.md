...
- [x] **AUTH-14**: Concurrent refresh across the plugin and the service is serialised by an `os.open(..., O_CREAT|O_EXCL)` lock with a stale-lock breaker; neither `threading.Lock` nor `fcntl.lockf` is used for this
  - **Left checked, with a defect recorded against it (measured 2026-08-23).** `RefreshLock.release()` ends with `os.unlink` inside `except OSError: pass`, and on Windows that clause swallows a real failure about one run in ten: while a waiting contender has the lock file open — `_recorded_session()` opens it on every poll, and CPython on Windows opens without `FILE_SHARE_DELETE` — the holder's unlink is refused with WinError 32, so **`release()` returns having released nothing** and the lock is orphaned until it ages past the 90-second lifetime. Measured over 300 replays with the swallowed exception captured; see deferred item 2, whose original "the test is timing-sensitive" diagnosis this disproves. **Still checked because the requirement's own sentence is about the mechanism, and on Linux — therefore on Android, therefore on the TCL — `unlink` succeeds regardless of open handles, so the serialisation guarantee holds on the target device.** What is defective on every platform is the swallow: a release that did not release reports success, and nothing anywhere asserts that the lock file is gone after `release()` returns. Belongs with `resources/lib/auth/lock.py`, not with the phase that found it
...
- [ ] **DIST-02**: A self-hosted Kodi repository is published over HTTPS using the `<dir>` schema and SHA-256 checksums, never MD5 and never the flat pre-Gotham layout. **The shape clauses are delivered; the "is published" clause is not, and that is why this is unchecked.** Built early on 2026-08-23 at the owner's request, outside any plan, while Phase 5 remains unplanned. Delivered: `repository.onedrive.kn/addon.xml` declares one `<dir>` with `<info>`, `<checksum verify="sha256">`, `<datadir>` and `<hashes>sha256</hashes>`, every URL https; `tools/build_repo.py` generates the tree and **refuses MD5 by name** — including `<hashes>true</hashes>`, which is Kodi's deprecated alias for it — rather than merely not choosing it; the flat pre-`<dir>` schema is refused with the reason (Kodi 20 removed it); and `tests/test_build_repo.py` asserts all of it against a real tree read back off disk, each property confirmed by mutation. **Not delivered: any of it being served.** GitHub Pages is not configured, `dist/` is gitignored so the tree is not in the index, and no Kodi has ever fetched a byte of it — so every assertion behind this requirement is an assertion about files on a build host, and none is about an HTTP response. Left unchecked on the rule that already leaves AUTH-20 and AUTH-06 unchecked: a conjunction with an unmet clause is not marked
- [ ] **DIST-03**: Installing the repository on Android TV requires entering a URL exactly once. **A measurement contradicts this requirement's premise, and it must be settled when Phase 5 is planned rather than implemented as written.** Deferred item 16 records that installing by URL was attempted on the TCL Android TV 12 and does not work: Kodi's *add source* browse needs a directory listing, and a plain file URL provides none. Pointing it at a repository is not a way round that, because **adding a repository is itself a zip install** — so the first install still costs a USB trip, and what the repository buys is every install after it. That is what `README.md` now states plainly, in those words, rather than implying one-click convenience. Two ways out, and choosing is planning work: restate the requirement as *one zip install, then nothing manual ever again*, or make the URL route real by publishing an HTML index at the repository path, since Kodi's HTTP directory reader parses `<a href>` links out of a page — **untested, recorded as a lead and not as a solution**. Same treatment DIST-01 and BROWSE-08 received when a measurement contradicted the text, except that here the correct replacement text is a design decision and this is not the place it gets taken
- [ ] **DIST-04**: An N to N+1 update is confirmed to arrive without the user pressing "Check for updates". **Untouched.** The early landing of 2026-08-23 delivered the mechanism an update would travel over and nothing that exercises it. This is the requirement with multi-hour latency, and the reason Phase 5 is scheduled to run concurrently with Phase 6 rather than last
- [ ] **DIST-05**: Published version numbers carry no pre-release suffix, which Kodi's version comparison sorts below the plain version. **The guard exists; the requirement says *published* and nothing is published.** `tools/build_repo.py` refuses to write a tree for a version that is not plain dotted numerals, and `test_a_pre_release_version_is_refused` drives a `4.5.6~beta1` manifest through it and asserts the refusal, so the failure mode is now impossible to reach by accident rather than merely avoided by care. 03-02 was right that the guard belongs on the publish path and not on the archive build: `tools/build_addon_zip.py` still passes the manifest's version through verbatim, because a pre-release archive that never reaches a repository harms nobody
...
| AUTH-14 | Phase 3 | Complete — with a defect recorded against `release()`, measured 2026-08-23. It swallows a failed unlink and can return having released nothing; Windows-only in symptom, so the serialisation guarantee holds on the target device, but the swallow is a defect everywhere. See the requirement's own note and deferred item 2 |
...
| DIST-02 | Phase 5 | Partial — the repository add-on, the generator and the tests landed early on 2026-08-23, outside any plan, with the manifest format checked against Kodi Omega rather than recalled. Every clause about the repository's *shape* is delivered and asserted; **the "is published" clause is not**, and nothing has been installed on a television. Phase 5 keeps the requirement |
| DIST-03 | Phase 5 | Blocked on a decision, not on work — **the requirement's premise is contradicted by a measurement.** Installing by URL does not work on the TCL, and adding a repository is itself a zip install, so "exactly one URL entry" describes something that cannot happen on the route available. Phase 5's planning either restates it as one zip install then nothing manual ever, or makes the URL route real with a published HTML index (untested lead). See DIST-03's own entry and deferred item 16 |
| DIST-04 | Phase 5 | Pending — untouched by the early landing, which delivered the mechanism and nothing that exercises it |
| DIST-05 | Phase 5 | Partial — `tools/build_repo.py` refuses to publish a non-plain version and a test asserts the refusal, so the failure is unreachable by accident. The requirement says *published*; nothing is published |
