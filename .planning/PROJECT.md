...
- [ ] Ship as `plugin.onedrive.kn` — a separate add-on that can never collide with the official `plugin.onedrive`, installs alongside it, and needs no migration because it has no prior profile
- [ ] Name the current maintainer in `addon.xml` while preserving `LICENSE.txt` and every upstream copyright notice, and credit the origin in `CREDITS.md`
...
| Ship under a new add-on id, `plugin.onedrive.kn` | Makes this a genuinely separate add-on: no version collision with the official `plugin.onedrive`, installable side by side, and its own profile directory | — Pending |
| No migration from v2.3.0 at all | Refresh tokens are bound to a `client_id` and must be discarded regardless. What remains — account display name and drive selection — is regenerated by signing in, so a migration phase would cost real work to save a user about ten seconds of setup they must do anyway | — Pending |
